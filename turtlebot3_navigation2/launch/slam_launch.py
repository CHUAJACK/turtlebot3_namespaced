...
from nav2_common.launch import HasNodeParams, ReplaceString, RewrittenYaml
...
def generate_launch_description():
    ...
    # slam_toolbox frames follow the launch namespace. Runs before RewrittenYaml.
    ...
    # Namespace-rewritten params. RewrittenYaml(root_key=namespace) wraps the whole
    # file under '<namespace>:' so top-level node keys like 'slam_toolbox:' resolve to
    # the fully-qualified node name '/<namespace>/slam_toolbox'. Without this, the bare
    # 'slam_toolbox:' key only matches '/slam_toolbox' and the namespaced node silently
    # falls back to its DEFAULT (un-namespaced) frames -> map/odom/base_footprint, which
    # breaks TF and makes the message filter drop every scan.
    ...
    declare_params_file_cmd = DeclareLaunchArgument(
        'params_file',
        default_value=os.path.join(bringup_dir, 'params', 'burger3310.yaml'),
